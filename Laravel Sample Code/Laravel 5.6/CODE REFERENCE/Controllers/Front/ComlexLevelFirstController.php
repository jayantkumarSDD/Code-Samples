...
Use App\Models\ComlexLevelFirstExam;
Use App\Models\ComlexLevelFirstExamQuestions;
use App\Models\QuestionsComment;
use App\Models\GenerateTestCategoriesDiscipline;
use Mail;
Use DB;
...
class ComlexLevelFirstController extends Controller {
    ...
    public function showQbankCategories() {
        $categories = ComlexLevelFirstQbankCategory::getQbankCategories();
        ...
        return View('/qbank/comlex_level_first/qbank_category', compact('categories','allowed_free_trial_questions','totalUsedQuestion'))->with('page_title', 'COMLEX Level 1 Qbank');
    }
    ...
    public function getCategoriesQuestionsModeWise($categories) {
        ...
        if (!empty($categories)):
            ...
            $usedQuestionIds = ComlexLevelFirstQuestion::getUsedQuestionsIdOfUser($user_id);
            $markedQuestionIds = ComlexLevelFirstQuestion::getMarkQuestionsIdOfUser($user_id);
            $incorrectQuestionIds = ComlexLevelFirstQuestion::getIncorrectQuestionsIdOfUser($user_id);


            foreach ($categories as $category):
                $category_id = $category['category_id'];
                $category['mode']['all'] = ComlexLevelFirstQuestion::where('category_id', $category_id)
                        ->where('status', 'Enabled')
                        ->count();


                $category['mode']['new'] = ComlexLevelFirstQuestion::where('category_id', $category_id)
                        ->whereNotIn('id', $usedQuestionIds)
                        ->where('status', 'Enabled')
                        ->count();

                $category['mode']['marked'] = ComlexLevelFirstQuestion::where('category_id', $category_id)
                        ->whereIn('id', $markedQuestionIds)
                        ->where('status', 'Enabled')
                        ->count();

                $category['mode']['incorrect'] = ComlexLevelFirstQuestion::where('category_id', $category_id)
                        ->whereIn('id', $incorrectQuestionIds)
                        ->where('status', 'Enabled')
                        ->count();

                $category['mode']['used'] = ComlexLevelFirstQuestion::where('category_id', $category_id)
    ...
    public function getTotalCountQuestionByCatDiscipline() {
        ...
        $result = ComlexLevelFirstQuestion::whereIn('category_id', $categories)
    ...
    public function resumeTest($test_id = NULL) {
        $user_id = \Auth::user()->id;
        $exam_data = ComlexLevelFirstExam::with('exam_question')
                        ->where('id', $test_id)
        ...
        if (!empty($exam_data)) {
            ...
            $filtered_questions = ComlexLevelFirstQuestion::whereIn('id', $questions)
                            ->orderBy(DB::raw("FIELD(id, $implode_questions)"))
                            ->get()->toArray();
            $questions_data = [];
            foreach ($filtered_questions as $question):
                if (!empty($question['parent'])) {
                    $base_question = ComlexLevelFirstBaseQuestion::where('id', $question['parent'])->first();
            ...
            return View('qbank/comlex_level_first/exam')->with('page_title', 'Comlex Qbank')->with('data', $response);
        } else {

            return Redirect::to('/student/comlex-level-1-review-exams');
        }
    }
    ...
    private function getFilteredQuestionsByMode($question_mode,$categories,$no_of_question,$user_id){
        ...
        {
            $filtered_questions = ComlexLevelFirstQuestion::whereIn('category_id', $categories)
                            ->where('status', 'Enabled')
    ...
    public function takeTest($test_id) {
        ...
        if ($exam_data->submit_type == 'paused') {
            return Redirect::to('/resume_exam/' . $test_id);
        } else if ($exam_data->submit_type == 'complete') {
            return Redirect::to('/review_exam/' . $test_id);
        } else {
            $test_data = GenerateTestCategoriesDiscipline::where('exam_id', $test_id)->first()->toArray();
                ...
                if ($check_count_question < $no_of_question):
                    if (!in_array($question['id'], $exist_question)):
                        if ($question['parent'] == 0) {
                            ...
                            $set_of_questions = ComlexLevelFirstQuestion::where('parent', $question['parent'])->where('status', 'Enabled')->get()->toArray();
            ...
            endforeach;
            ComlexLevelFirstExamQuestions::insert($exam_questions);
            $response['test_mode'] = $test_mode;
            $response['test_id'] = $test_id;
    ...
    private function validateAccess($vars){
        ...
        if(\Auth::user()->is_free_trial_user && !\Auth::user()->hasOMMAccess){
            ...
            $allowed_omm_questions = ComlexLevelFirstQbankCategory::join('comlex_level_1_questions', 'comlex_level_1_category.id', '=', 'comlex_level_1_questions.category_id')
                                            ->where('comlex_level_1_category.status','=','Enabled')
                                            ->where('comlex_level_1_category.is_omm_category','=','yes')
        ...
        else if(!\Auth::user()->membership_access['level_1_qbank']['hasAccess']){
    ...
    public function createTest() {
        ...
        /*         * ********************************************* */
        $comlexLevelFirstExam = new ComlexLevelFirstExam;
        $comlexLevelFirstExam->user_id = $user_id;
        $comlexLevelFirstExam->test_mode = $test_mode;
        $comlexLevelFirstExam->question_mode = $question_mode;
        $exam = $comlexLevelFirstExam->save();
        /*         * ********************************************* */

        $generateTestCategoriesDiscipline = new GenerateTestCategoriesDiscipline;
        $generateTestCategoriesDiscipline->exam_id = $comlexLevelFirstExam->id;
...
//        $generateTestCategoriesDiscipline->discipline = implode(',', $input['discipline']);
        ...
        return Redirect::to('/take/test/' . $comlexLevelFirstExam->id);
    }
        ...
        if (!empty($user)) {
            ...
            try {
                $status = Mail::queue('emails.comments', $data, function($message) {
                            $message->to('admin@omtreview.com', 'OMTREVIEW')->subject('OMT Comlex Level 1 Qbank Comments');
    ...
    public function saveTest() {
        ...
        if (!empty($examData)) {
            ...
            foreach ($questions as $key => $value):
                ...
                ComlexLevelFirstExamQuestions::where('exam_id', $exam_id)
                        ->where('question_id', $key)
                        ->update($update);
            endforeach;
            $comlexLevelFirstExam = ComlexLevelFirstExam::find($exam_id);
            $comlexLevelFirstExam->total_time_spent = $total_time;
            $comlexLevelFirstExam->submit_type = $submit_type;
            $comlexLevelFirstExam->last_resume_index = $last_resume_index;
            $comlexLevelFirstExam->save();
            return true;
        }
    }
    ...
    public function submitTest() {
        $this->saveTest();
        return Redirect::to('/student/comlex-level-1-review-exams')->with('page_title','Review My Exam');
    }
    ...
    public function reviewExams() {
        $user_id = \Auth::user()->id;
        $exams = ComlexLevelFirstExam::with('exam_question')
                ->where('user_id', $user_id)
                ->orderBy('id', 'desc')
                ->paginate(10);
        ...
        foreach ($exams as $exam):
            if (!$exam->exam_question->isEmpty()):
                ...
                foreach ($exam->exam_question as $exam_question):
                    $question = ComlexLevelFirstQuestion::find($exam_question->question_id);
                ...
                if (count($category) > 1) {
                    ...
                    $category = ComlexLevelFirstQbankCategory::find($category[0]);
        ...
        endforeach;
        return View('user.review_exams')->with('exams', $exams)->with('page_title','Review My Exam (COMLEX Level 1)');
    }
