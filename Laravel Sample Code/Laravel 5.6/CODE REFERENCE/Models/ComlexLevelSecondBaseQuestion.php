...
class ComlexLevelSecondBaseQuestion extends Model {
     ...
     * @var string
     */
    protected $table = 'comlex_level_2_base_questions';
    ...
    public static function addUpdateQuestion( $vars = null ){
        $question = ComlexLevelSecondBaseQuestion::firstOrNew(['id' => isset($vars['id']) ? $vars['id'] : '']);
    ...
    public static function getQuestions($limit = NULL){
        $questions = ComlexLevelSecondBaseQuestion::orderBy('id', 'desc')->paginate($limit);
        return $questions;
    }
    
    public static function getSetsQuestions($limit = NULL){
        $questions = ComlexLevelSecondBaseQuestion::with('mainQuestions')
                            ->where('type','sets')
                            ->orderBy('id', 'desc')
                            ->paginate($limit);
        return $questions;
    ...
    {
        return $this->hasMany('App\Models\ComlexLevelSecondQuestion','parent');
    }
