...
class ComlexLevelFirstQuestion extends Model {
     ...
     * @var string
     */
    protected $table = 'comlex_level_1_questions';
    ...
    public static function addUpdateSingleAnswerQuestions($vars) {
        $question = ComlexLevelFirstQuestion::firstOrNew(['id' => isset($vars['id']) ? $vars['id'] : '']);
        ...
        if (!empty($vars['parent'])){
            $child_question_count = ComlexLevelFirstQuestion::where('parent',$vars['parent'])->count();
            ComlexLevelFirstQuestion::where('parent',$vars['parent'])->update(['question_count' => $child_question_count]);
        }
    ...
    public static function getAllWithAllAssociated($limit = NULL) {
        $records = DB::table('comlex_level_1_questions')
                ->select(\DB::raw('comlex_level_1_questions.*,(select title from comlex_level_1_category where comlex_level_1_questions.category_id = comlex_level_1_category.id ) as category,(select type from comlex_level_1_base_questions where comlex_level_1_questions.parent = comlex_level_1_base_questions.id ) as type'))
                ->orderBy('id', 'desc')
                ->paginate($limit);
        return $records;
    }
    ...
    public static function doSearchQuestions($keyword = NULL, $limit = NULL) {
        $result = DB::table('comlex_level_1_questions')
                ->select(DB::raw('comlex_level_1_questions.*,(select title from comlex_level_1_category where comlex_level_1_questions.category_id = comlex_level_1_category.id ) as category'))
    ...
    public static function doFilterQuestions($keyword = NULL, $d1 = NULL, $d2 = NULL, $category = NULL, $id = NULL, $limit = NULL) {
        ...
        $query = DB::table('comlex_level_1_questions');
        
        $query->select(DB::raw('comlex_level_1_questions.*,(select title from comlex_level_1_category where comlex_level_1_questions.category_id = comlex_level_1_category.id ) as category, (select type from comlex_level_1_base_questions where comlex_level_1_questions.parent = comlex_level_1_base_questions.id ) as type'));
    ...
    public static function getUsedQuestionsIdOfUser($user_id = NULL) {
        $data = DB::table('comlex_level_first_exam')
                ->join('comlex_level_first_exam_questions', 'comlex_level_first_exam.id', '=', 'comlex_level_first_exam_questions.exam_id')
                ->where('comlex_level_first_exam.user_id', $user_id)
                ->select('comlex_level_first_exam_questions.question_id')
                ->groupBy('comlex_level_first_exam_questions.question_id')
                ->get();
    ...
    public static function getIncorrectQuestionsIdOfUser($user_id = NULL)
    {
        $data = DB::table('comlex_level_first_exam')
                ->join('comlex_level_first_exam_questions', 'comlex_level_first_exam.id', '=', 'comlex_level_first_exam_questions.exam_id')
                ->where('comlex_level_first_exam.user_id', $user_id)
                ->where('comlex_level_first_exam_questions.is_correct','0')
                 ->where('comlex_level_first_exam_questions.isSubmited','1')
                ->select('comlex_level_first_exam_questions.question_id')
                ->groupBy('comlex_level_first_exam_questions.question_id')
                ->get();
        $data = json_decode(json_encode($data), true);
    ...
    public static function getMarkQuestionsIdOfUser($user_id = NULL)
    {
        $data = DB::table('comlex_level_first_exam')
                ->join('comlex_level_first_exam_questions', 'comlex_level_first_exam.id', '=', 'comlex_level_first_exam_questions.exam_id')
                ->where('comlex_level_first_exam.user_id', $user_id)
                ->where('comlex_level_first_exam_questions.is_mark','1')
                ->select('comlex_level_first_exam_questions.question_id')
                ->groupBy('comlex_level_first_exam_questions.question_id')
                ->get();
        $data = json_decode(json_encode($data), true);
