...
class ComlexLevelFirstQbankCategory extends Model {
     ...
     * @var string
     */
    protected $table = 'comlex_level_1_category';
    ...
    public static function addUpdateQbankCategory($vars) {
        $cat = ComlexLevelFirstQbankCategory::firstOrNew(['id' => isset($vars['id']) ? $vars['id'] : '']);
    ...
    public static function doSearch($keyword) {
        $result = ComlexLevelFirstQbankCategory::orWhere('id', 'LIKE', "%$keyword%")
    ...
    public static function get_all_parent_categories() {
        $parent_categories = ComlexLevelFirstQbankCategory::where('parent_id',0)
    ...
    public static function getQbankCategoryById($id) {
        return ComlexLevelFirstQbankCategory::find($id)->toArray();
    }
    ...
    public static function get_qbank_category_with_child($id = 0, $pass = 0, $selected = 0, $exclude_id = null) {
        $cats = ComlexLevelFirstQbankCategory::where('parent_id', '=', $id)->get();
        $html = '';
        if (!$cats->isEmpty()) {
            foreach ($cats as $val):
                if ($val->id != $exclude_id):
                    ...
                    $html .= ComlexLevelFirstQbankCategory::get_qbank_category_with_child($val->id, $pass + 1, $selected, $exclude_id);
                endif;
            endforeach;
        }
        return $html;
    ...
    public static function get_qbank_category_with_child_array($id = 0, $pass = 0, $selected = 0, $exclude_id = null) {
        $cats = ComlexLevelFirstQbankCategory::where('parent_id', '=', $id)->get()->toArray();
        $category = array();
        if (!empty($cats)) {
            foreach ($cats as $key => $val):
                ...
                $category[$key]['child'] = ComlexLevelFirstQbankCategory::get_qbank_category_with_child_array($val['id'], $pass + 1, $selected, $exclude_id);
    ...
    public static function get_qbank_category_with_child_ids($id = 0, $pass = 0, $selected = 0, $exclude_id = null) {
        $cats = ComlexLevelFirstQbankCategory::where('parent_id', '=', $id)->where('status', '=', 'Enabled')->get()->toArray();
        $category = '';
        $category .= $id . ',';
        if (!empty($cats)) {
            foreach ($cats as $key => $val):
                ...
                $category .= ComlexLevelFirstQbankCategory::get_qbank_category_with_child_ids($val['id'], $pass + 1, $selected, $exclude_id);
    ...
    public static function getParentNameByCategory($multi_id = NULL) {
        $data = ComlexLevelFirstQbankCategory::whereIn('id', $multi_id)->get()->toArray();
        ...
        if (count($cat) == 1 && $cat[0] == 0) {
            ...
            $return .= ComlexLevelFirstQbankCategory::getParentNameByCategory($cat);
        } else {
            $return .= 'Multiple';
        }
    ...
    public static function getParentIdsByCategoryIdsArray($ids = NULL) {
        $data = ComlexLevelFirstQbankCategory::whereIn('id', $ids)->get();
        ...
        if (!empty($data)):
            foreach ($data as $key => $value):
                ...
                if ($value->parent_id != 0) {
                    $return .= ComlexLevelFirstQbankCategory::getParentIdsByCategoryIdsArray(array($value->parent_id));
                }
            endforeach;
        endif;
        return $return;
    ...
    public static function getParentNameByCategoryArray($id = NULL) {
        ...
        if ($data->parent_id != 0) {
            $return [] = ComlexLevelFirstQbankCategory::getParentNameByCategoryArray($data->parent_id);
        }
    ...
    public static function getParentNameByCategoryIdWithoutSelf($id = NULL) {
        $data = ComlexLevelFirstQbankCategory::find($id);
        $return = [];
        if ($id !== $data->id):
            $return [$data->parent_id] = isset($data->title) ? $data->title : '';
        endif;
        if ($data->parent_id != 0) {
            $return [$data->parent_id] = ComlexLevelFirstQbankCategory::getParentNameByCategoryArray($data->parent_id);
        }
        ...
        if (!$cats->isEmpty()) {
            foreach ($cats as $val):
                ...
                $html .= '<td>
                                     ...
                                     ';
                $html .= '<td>
			<a href="/admin/comlex_level_first_qbankcategory?id=' . $val->id . '"><i class="fa fa-edit"></i></a>
			&nbsp; &nbsp; &nbsp;<a href="javascript:void(0);" data-rel="' . $val->id . '" class="deleteRecord"><i class="fa fa-trash-o fa-lg"></i></a>
		</td>';
                $html .= '</tr>';
                $html .= ComlexLevelFirstQbankCategory::get_qbank_category_with_child_tr($val->id, $pass + 1, 0);
    ...
    public static function getQbankCategories(){
        ...
        $categories = ComlexLevelFirstQbankCategory::where('status','Enabled')
